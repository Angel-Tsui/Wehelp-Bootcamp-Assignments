...
# your code here
# 取得車廂的長度
    train_length=len(status)
    # 取得所有車廂的空位數量 seats
    car_availableSeats=[]
    for i in range(train_length):
        open_seats=seats[i]
        # 如果空位數量比 number 大，而且車廂狀態 status 是 1 的車廂記錄下來
        if open_seats >= number and status[i] == 1:
            car_availableSeats.append(open_seats)
    # 如果 car_availableSeats 裏沒有 item，代表沒有符合的車廂，直接印出 -1
    if len(car_availableSeats) == 0:
        print("-1")
    # 如果 car_availableSeats 裏有 item，找出最相近的車廂，印出車廂編號
    else:
        enter=min(car_availableSeats)
        ticket=seats.index(enter)
        print(ticket)
